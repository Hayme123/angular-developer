...
export class CustomSlider {
  ...
  public readonly changed = output<number>({ alias: 'valueChanged' });
}
```
...
const subscription = componentRef.instance.valueChanged.subscribe((value: number) => {
  // Handle the typed value through the owning feature.
});
...
export class LegacyExample {
  @Output() public readonly valueChanged = new EventEmitter<number>();

  // With alias
  @Output('customEventName') public readonly changed = new EventEmitter<void>();
}
```
